...
#include "../../Inc/LCD/LCD_int.h"
#include "../../Inc/LCD/LCD_pri.h"
#include "../../Inc/LCD/LCD_cfg.h"
...
void LCD_SendCommand(u8 cmd){
	...
	DIO_vSetPinVal(ENPort, PIN2_ID, VAL_LOW); // EN low
	DIO_vSetPinVal(RSPort, PIN3_ID, VAL_LOW); // RS low for command mode

	DIO_vSetPinVal(DataPort, DataPin1, GET_BIT(cmd, 4));
	DIO_vSetPinVal(DataPort, DataPin2, GET_BIT(cmd, 5));
	DIO_vSetPinVal(DataPort, DataPin3, GET_BIT(cmd, 6));
	DIO_vSetPinVal(DataPort, DataPin4, GET_BIT(cmd, 7));

	// Trigger
	DIO_vSetPinVal(ENPort, PIN9_ID, VAL_HIGH); // EN high
	_delay_ms(2);
	DIO_vSetPinVal(ENPort, PIN9_ID, VAL_LOW);
	_delay_ms(2);

	DIO_vSetPinVal(DataPort, DataPin1, GET_BIT(cmd, 0));
	DIO_vSetPinVal(DataPort, DataPin2, GET_BIT(cmd, 1));
	DIO_vSetPinVal(DataPort, DataPin3, GET_BIT(cmd, 2));
	DIO_vSetPinVal(DataPort, DataPin4, GET_BIT(cmd, 3));
...
void LCD_vInit(){
	...
	/*1- Dio Initialize*/
	DIO_vSetPinDir(RSPort, RSPIN, OUTPUT_PUSH_PULL_LOW_FREQ);
	DIO_vSetPinDir(ENPort, ENPIN, OUTPUT_PUSH_PULL_LOW_FREQ);

	DIO_vSetPinDir(DataPort, DataPin1, OUTPUT_PUSH_PULL_LOW_FREQ);
	DIO_vSetPinDir(DataPort, DataPin2, OUTPUT_PUSH_PULL_LOW_FREQ);
	DIO_vSetPinDir(DataPort, DataPin3, OUTPUT_PUSH_PULL_LOW_FREQ);
	DIO_vSetPinDir(DataPort, DataPin4, OUTPUT_PUSH_PULL_LOW_FREQ);

	_delay_ms(35);
	//FIRST 4 BITS SENT (function set)
	DIO_vSetPinVal(ENPort,ENPIN ,VAL_LOW);
	DIO_vSetPinVal(ENPort,RSPIN ,VAL_LOW);

	//FIRST 4 BITS SENT (function set)
	DIO_vSetPinVal(DataPort,DataPin1,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin2,VAL_HIGH);
	DIO_vSetPinVal(DataPort,DataPin3,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin4,VAL_LOW);
	...
	//last 3 bits
	DIO_vSetPinVal(DataPort,DataPin1,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin2,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin3,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin4,VAL_HIGH);
	...
	DIO_vSetPinVal(ENPort,RSPIN ,VAL_LOW);
	_delay_ms(1);
	DIO_vSetPinVal(DataPort,DataPin1,VAL_LOW); //B
	DIO_vSetPinVal(DataPort,DataPin2,VAL_LOW); //C
	DIO_vSetPinVal(DataPort,DataPin3,VAL_HIGH); //D
	DIO_vSetPinVal(DataPort,DataPin4,VAL_HIGH);
	...
	DIO_vSetPinVal(ENPort,RSPIN ,VAL_LOW);
	_delay_ms(1);
	DIO_vSetPinVal(DataPort,DataPin1,VAL_HIGH);
	DIO_vSetPinVal(DataPort,DataPin2,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin3,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin4,VAL_LOW);
	DIO_vSetPinVal(ENPort,ENPIN ,VAL_HIGH);
	...
	DIO_vSetPinVal(DataPort,DataPin1,VAL_LOW);
	DIO_vSetPinVal(DataPort,DataPin2,VAL_HIGH);
	DIO_vSetPinVal(DataPort,DataPin3,VAL_HIGH);
	DIO_vSetPinVal(DataPort,DataPin4,VAL_LOW);
...
void LCDdispchar(u8 u8Data) {
	...
	DIO_vSetPinVal(RSPort, RSPIN, VAL_HIGH); // RS high for data mode

	DIO_vSetPinVal(DataPort, DataPin1,GET_BIT(u8Data, 4));
	DIO_vSetPinVal(DataPort, DataPin2, GET_BIT(u8Data, 5));
	DIO_vSetPinVal(DataPort, DataPin3, GET_BIT(u8Data, 6));
	DIO_vSetPinVal(DataPort, DataPin4, GET_BIT(u8Data, 7));

	// Trigger
	DIO_vSetPinVal(ENPort, ENPIN, VAL_HIGH); // EN high
	_delay_ms(2);
	DIO_vSetPinVal(ENPort, ENPIN, VAL_LOW);
	_delay_ms(2);

	DIO_vSetPinVal(DataPort, DataPin1, GET_BIT(u8Data, 0));
	DIO_vSetPinVal(DataPort, DataPin2, GET_BIT(u8Data, 1));
	DIO_vSetPinVal(DataPort, DataPin3, GET_BIT(u8Data, 2));
	DIO_vSetPinVal(DataPort, DataPin4, GET_BIT(u8Data, 3));
...
void lcd_write_data(u8 A_u8Data){
	...
	DIO_vSetPinVal(DataPort, DataPin1, GET_BIT(A_u8Data, 4));
	DIO_vSetPinVal(DataPort, PIN1_ID, GET_BIT(A_u8Data, 5));
	DIO_vSetPinVal(DataPort, PIN2_ID, GET_BIT(A_u8Data, 6));
	DIO_vSetPinVal(DataPort, PIN4_ID, GET_BIT(A_u8Data, 7));

	// Trigger
	DIO_vSetPinVal(ENPort, PIN2_ID, VAL_HIGH); // EN high
	_delay_ms(2);
	DIO_vSetPinVal(ENPort, PIN2_ID, VAL_LOW);
	_delay_ms(2);

	DIO_vSetPinVal(DataPort, DataPin1, GET_BIT(A_u8Data, 0));
	DIO_vSetPinVal(DataPort, PIN1_ID, GET_BIT(A_u8Data, 1));
	DIO_vSetPinVal(DataPort, PIN2_ID, GET_BIT(A_u8Data, 2));
	DIO_vSetPinVal(DataPort, PIN4_ID, GET_BIT(A_u8Data, 3));
